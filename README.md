...
**Merge similar SQL.** Viewer-wide toggle (on by default) shared by the statement, ASH, and Latency-modes tabs. When on, IN-lists, VALUES lists, and bind-position variables are collapsed so variants share one canonical shape (see **Query normalization** below). Table and column names stay distinct. Off, every statement stands on its own and the grouping / recurring-template controls disappear. YCQL only collapses `IN (...)` lists (`?` binds, so `$N` rewriting does not apply). Grouping is browser-only (no snapshot-format change) and honors **cumulative/delta** mode plus any active ASH filter.

**Canonicalized query.** With Merge on, statement and Latency-modes tables relabel the query column to **canonicalized query** and show the normalized text (plain text, not a per-query ASH link — **queryid** still links when that column is present). On Latency modes, **queryid** sits immediately to the right of that column. ASH query-bearing tables use **canonical query** (or **canonicalized query** on Database & Query) as the grouping key instead of raw SQL / `query_id`. The ASH Top 50 becomes **Table/Index + Canonical Query + Wait_Event**. Database & Query omits **query_id** in this mode (membership is on Recurring query templates).

**Group by query template** (statement panels only; requires Merge on). Collapses the **Top 25** to one row per canonical query **per database**. The **dbname** column and Top 25 heading stay. Works in both cumulative and delta modes: raw counters are aggregated first so collapsed rows flow through the existing time-% / delta pipeline.

**Show recurring query templates** (optional; requires Merge on). Lists templates with more than one member: canonical query, aggregated **total time** / **time %** (or **calls/s** in delta) for statements, **Active Sessions/sec** / **Load %** for ASH, and the full ranked member `queryid` list. Sorting the summary by a metric column re-ranks those members by the same metric. Latency-modes membership is computed from every analyzed statement so rank stays stable under **min tier** and **flagged only**. There is no **template_id rank/count** column on the statement Top 25, Latency-modes main table, ASH Top 50, or Database & Query tables.
...
5. **Latency modes** (only when `latency_histograms` is present) – Statements whose per-call latency distribution is **bimodal/multimodal**, with **tier**, **calls**, **bimodality coefficient**, **peaks**, latency **spread**, and **gap** (every adjacent mode split, comma-separated when there are 3+ peaks). With **Merge similar SQL** on, the query column is **canonicalized query** (queryid to its right); **Show recurring query templates** optionally lists multi-member templates. Uses the same cumulative/delta convention as the statement tabs. When a snapshot was captured with **`--snapshot-latency-analysis`**, the tab loads the precomputed **dip-confirmed** report — real `dip_p` and FDR-corrected tiers with a **dip-confirmed** badge, no statistics run in the browser. Otherwise the browser runs the shape-based stages only; the Hartigan **dip test is not run in the browser**, so flagged rows show as **`unconfirmed`** — capture with **`--snapshot-latency-analysis`** (and `pip install 'ybtop[histogram]'`) for dip-confirmed tiers.
