...
| **`ybtop histogram --data-dir DIR`** | **Offline, no database.** Analyzes the **`latency_histograms`** stored in snapshot files (captured with `watch --snapshot-latency-histograms`) to flag statements whose per-call latency is **bimodal/multimodal**. Supports **`--mode`** `cumulative` / `delta` / `auto`. Add **`--write [--all]`** to persist a precomputed **`ybtop.latency.*.json`** analysis sidecar (dip-confirmed) for offline browser viewing. Requires the optional `[histogram]` extra. |
...
**Latency multimodality (`ybtop histogram`)**: Flags: **`--mode`** picks `cumulative` (the snapshot's totals), `delta` (subtract the prior snapshot's per-bucket counts), or `auto` (delta when a prior snapshot has histogram data, else cumulative). Select a snapshot with **`--index`** (1-based; negative counts from the end; default latest) or **`--snapshot TS`**. **`--min-calls`** (default `30`) ignores low-traffic statements; **`--min-tier`** (default `high`; `very_high` / `high` / `moderate` / `unconfirmed` / `all`) sets the reporting floor; **`--no-fdr-correct`** / **`--fdr-q`** control the Benjamini-Hochberg correction (on by default); **`--flagged-only`** and **`--json`** shape the output. Without the `diptest` package the confirmatory dip test is skipped and shape-flagged rows are reported as **`unconfirmed`** (the same behavior as the browser viewer). **`--write [--all]`** persists the full report (cumulative + delta, unfiltered) as an **`ybtop.latency.*.json`** sidecar recorded in the manifest, so the viewer's **Latency modes** tab shows the same dip-confirmed tiers offline.
...
- **`ybtop.latency.<ts>.json`** (sidecar) – present when the snapshot was captured with **`--snapshot-latency-analysis`** or a sidecar was written via **`ybtop histogram --write`**: the precomputed dip-confirmed report (`cumulative` + `delta`, unfiltered) that the viewer's **Latency modes** tab loads for offline confirmed tiers. Referenced from the manifest entry's **`latency_analysis`** field and pruned alongside its snapshot by retention GC.  
...
5. **Latency modes** (only when `latency_histograms` is present) – Statements whose per-call latency distribution is **bimodal/multimodal**, with **tier**, **calls**, **bimodality coefficient**, **peaks**, latency **spread**, and the **gap** between speed clusters, plus a **recurring query templates** summary. Uses the same cumulative/delta convention as the statement tabs. When a snapshot was captured with **`--snapshot-latency-analysis`** (or a sidecar was written via **`ybtop histogram --write`**), the tab loads the precomputed **dip-confirmed** report — real `dip_p` and FDR-corrected tiers with a **dip-confirmed** badge, no statistics run in the browser. Otherwise the browser runs the shape-based stages only; the Hartigan **dip test is not run in the browser**, so flagged rows show as **`unconfirmed`** — use **`ybtop histogram`** (with the `[histogram]` extra) for dip-test confirmation and FDR-corrected tiers.
