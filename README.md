...
**Merging similar SQL**  
An application that sends `IN ($1, $2)` on one call and `IN ($1, $2, $3)` on the next gets a separate `queryid` for each variant, so a single hot query can be scattered across a dozen rows that each look too small to matter. **Merge similar SQL** — one toggle, shared by the statement, ASH, and Latency-modes tabs — treats those variants as the same shape: bind positions, `IN` value lists, and bulk `VALUES` lists collapse, while table and column names are left alone so genuinely different queries stay apart. It starts **off**, so nothing is merged until you ask. YCQL only needs the `IN (...)` step, since it uses `?` bind markers. All of this happens in the browser on the snapshot already loaded, and it follows whatever cumulative/delta mode or ASH filter you have set.

Turn it on and the statement and Latency-modes tables retitle their query column to **canonicalized query** and show the collapsed text; **queryid** stays beside it and remains the way into ASH. The ASH tab goes further and groups *by* that text, so its Top 50 reads **Table/Index + Canonical Query + Wait_Event** and the per-database and per-table rollups follow suit — Database & Query drops **query_id** there, since one row now speaks for several statements.

On the statement tabs, **Group by query template** takes the last step and folds the **Top 25** into one row per canonical query, still split by **dbname**. Counters are summed before the collapse, so time %, calls/s, and delta arithmetic behave exactly as they do ungrouped.

**Show recurring query templates** adds a table of the shapes that really do have several members: the canonical query, its combined cost (**total time** / **time %**, or **calls/s** in delta mode; **Active Sessions/sec** / **Load %** in ASH), and every member `queryid` in rank order. That ranking follows the column you sort by — sort the summary by calls/s and the members reorder by calls/s. On Latency modes the membership is taken from every analyzed statement, so the **min tier** and **flagged only** filters change which templates you see, never what a template contains.

The collapsing itself is `normalize_query_template`, kept byte-identical between the CLI and the browser so both group SQL the same way. It strips the per-call noise — bind positions, list lengths, comment tags an application stamps onto each execution — and leaves the parts that change a plan. Planner hints beginning with `/*+` (pg_hint_plan / YSQL) survive, so two queries that differ only in their hints stay separate. Step by step:
...
5. **Latency modes** (only when `latency_histograms` is present) – Statements whose per-call latency distribution is **bimodal/multimodal**, with **tier**, **calls**, **bimodality coefficient**, **peaks**, latency **spread**, and **gap** (every adjacent mode split, comma-separated when there are 3+ peaks). Merging SQL here swaps the query column for **canonicalized query** with **queryid** beside it, and the recurring-templates summary shows which flagged statements are really the same shape. Uses the same cumulative/delta convention as the statement tabs. When a snapshot was captured with **`--snapshot-latency-analysis`**, the tab loads the precomputed **dip-confirmed** report — real `dip_p` and FDR-corrected tiers with a **dip-confirmed** badge, no statistics run in the browser. Otherwise the browser runs the shape-based stages only; the Hartigan **dip test is not run in the browser**, so flagged rows show as **`unconfirmed`** — capture with **`--snapshot-latency-analysis`** (and `pip install 'ybtop[histogram]'`) for dip-confirmed tiers.
