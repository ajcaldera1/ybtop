...
1. **pg_stat_statements** – Merged cluster view: **calls**, **time (ms)**, **time %**, **mean time**, **query**, optional **dbname**, **per-metric / call** for rows and DocDB fields when present, and **queryid**. If the previous manifest entry is loadable, a **delta** mode compares consecutive snapshots. The **queryid** column and the query text both link to the **ASH** tab filtered to that **`query_id`**. With **Merge similar SQL** on, the **queryid** column is dropped (one row now stands for several statements) but the query text still links — it scopes ASH to the heaviest member of that family.  
...
Turn it on and the statement tables fold the **Top 25** into one row per canonical query (YSQL still split by **dbname**), with the query column retitled **canonical query**. There is no separate “group by template” control — merging *is* the grouping. Counters are summed first, so time %, calls/s, and delta arithmetic behave as they do ungrouped. The **queryid** column leaves the Top 25 in that mode; the query text remains an ASH deep link, scoped to the member with the most execution time. Latency-modes keeps one row per statement but shows the same collapsed text; both **canonical query** and **queryid** deep-link to ASH for that statement. The ASH tab groups *by* that text, so its Top 50 reads **Table/Index + Canonical Query + Wait_Event** and the Database & Query / Table/Index & Query rollups follow suit. Database & Query also drops **query_id** there, since one row now speaks for several statements.
