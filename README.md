...
**Query-template grouping (statement + ASH tabs).** The **pg_stat_statements**, **ycql_stat_statements**, and **ASH** tabs share the Latency-modes tab's query-template normalization — value-list **`IN (...)`** arity (subquery `IN (SELECT …)` predicates are left intact), bulk **`VALUES (...),(...),…`** row-lists (e.g. `UPDATE … FROM (VALUES (...))`), and per-call comments are collapsed while table/column names are preserved, so queries that differ only in their bind-variable or row counts fold into one template. Each panel adds a **template_id rank/count** column (`LABEL rank/count` — blank when the row has no recurring-template match), a **Recurring query templates** summary listing templates with more than one member (aggregated by `total time`/`time %` for statements, `Active Sessions/sec`/`Load %` for ASH) with each template's full ranked member list, and (on the statement panels) a **Group by query template** toggle that collapses the table to one row per template with a **`queries`** count. The ASH Top 50 stays the dimensional wait-event breakdown; templates annotate that table via **template_id rank/count** and the recurring summary rather than replacing it. Template ids read `PRIMARY_TABLE_SQLVERB_VARIANT` (e.g. `RES_RELATE_DELETE_1`), with a numeric suffix disambiguating collisions. A viewer-wide **Merge similar SQL** toggle turns normalization off (IN-lists, VALUES lists, and bind position variables are collapsed when on); with it off each statement stands on its own and the template_id rank/count column, recurring-template summary, and grouping toggle are hidden. Grouping honors the current **cumulative/delta** mode and any active ASH filter. It runs entirely in the browser (no snapshot-format change); YCQL only normalizes `IN (...)` arity because it uses `?` bind markers.  
