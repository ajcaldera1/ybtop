...
| **`ybtop histogram --data-dir DIR`** | **Offline, no database.** Analyzes the **`latency_histograms`** stored in snapshot files (captured with `watch --snapshot-latency-histograms`) to flag statements whose per-call latency is **bimodal/multimodal**. Supports **`--mode`** `cumulative` / `delta` / `auto`. Requires the optional `[histogram]` extra. |
