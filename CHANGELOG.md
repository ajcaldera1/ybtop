...
- **Browser (query-template grouping):** The statement, ASH, and Latency-modes tabs can treat bind-count / IN-list / VALUES-list variants as one shape. **Merge similar SQL** (off by default) is the viewer-wide switch — there is no separate group-by control. With it on, query columns become **canonical query**, YSQL/YCQL Top 25s fold to one row per canonical query (YSQL still split by **dbname**), and ASH groups the Top 50 by **Table/Index + Canonical Query + Wait_Event** (unresolved SQL stays split by `query_id` so distinct sessions do not collapse). Clicking a merged Top 25 query opens `?view=ash&query=…&canonicalize=t`, expands the representative id to every `query_id` in that snapshot-local canonical family, and aggregates statement metrics plus ASH breakdowns across the family; YSQL links also carry **dbname**, while unresolved ids fall back to the ordinary one-id report. Family rows label their heaviest member as **representative query_id**. With Merge off, blank-query ASH rows stay one bucket, as before. Latency-modes remains one row per statement; with Merge on, **canonical query** uses the same family ASH link, while **queryid** stays a single-id report. **Show recurring query templates** (gated on merging) optionally lists multi-member shapes with a ranked `queryid` list that follows the column you sort by. Membership for Latency-modes templates is taken from every analyzed statement so min-tier / flagged-only filters do not rewrite a template. YCQL uses `?` binds, so the `$N` step never fires, and CQL has no multi-row `VALUES` lists; it still gets `IN (...)` collapsing, comment stripping, and single-row `VALUES (...)` canonicalization (which merges literal inserts). Grouping and family indexes are browser-only; `normalize_query_template` stays byte-identical between CLI and viewer, and the snapshot format is unchanged.
