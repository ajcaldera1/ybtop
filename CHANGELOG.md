...
- **Query-template normalization (CLI + viewer):** `normalize_query_template` (and its byte-identical browser twin) now also collapses a bulk **`VALUES (...),(...),…`** row-list — any number of rows, one level of nested parens allowed per row for casts/function calls — to a canonical **`VALUES (...)`**. This folds multi-row upserts/bulk updates such as `UPDATE t AS x SET … FROM (VALUES (...)) WHERE …` into a single template regardless of row count, affecting both `ybtop histogram` grouping and the viewer's template grouping.
