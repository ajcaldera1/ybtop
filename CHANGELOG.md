...
- **Browser (query-template grouping across statement/ASH panels):** The **pg_stat_statements**, **ycql_stat_statements**, and **ASH** tabs now expose the same query-template grouping the Latency-modes tab uses (IN-lists, bulk **`VALUES (...),(...),…`** row-lists, and per-call comments normalized; table/column names preserved). Each panel gains a **template_id rank/count** column tagging rows that belong to a recurring template with `LABEL rank/count` (blank otherwise), a **Recurring query templates** summary table (templates with more than one member, aggregated by `total time`/`time %` for statements and `Active Sessions/sec`/`Load %` for ASH) showing each template's full ranked member list — member ranks track the summary table's active sort column, so sorting by `calls/s`, `Load %`, `best tier`, etc. re-ranks each template's queryids by that metric and labels them with it — and (on the statement panels) a **Group by query template** toggle that collapses the main table to one row per template (with a `queries` count). ASH keeps the dimensional Top 50 (table/index + query + wait event) and uses templates only for annotation via **template_id rank/count** and the recurring summary. Template ids are semantic — `PRIMARY_TABLE_SQLVERB` plus a shape suffix where one applies (`ORDERS_SELECT_JOIN`, `ORDERS_INSERT_ONCONFLICT_UPDATE`), numeric suffix on collisions — and a viewer-wide **Merge similar SQL** toggle disables normalization entirely, hiding the template_id rank/count column, the recurring summary, and the grouping toggle. Grouping works in both **cumulative** and **delta** modes, aggregating raw counters so collapsed rows flow through the existing time-% / delta pipeline. YCQL notes that only `IN (...)` lists are normalized (it uses `?` bind markers). Grouping is browser-only — the normalizer is kept byte-identical to the Python `normalize_query_template`, so no snapshot format change is required.
